if(NOT CAKE_PROJECT_INCLUDED)
  ...
  macro(_cake_determine_project_dir)
    ...
    if(CMAKE_SCRIPT_MODE_FILE)
      ...
      # 1. attempt current dir
      # 2. attempt CAKE_PROJECT_DIR
      if(EXISTS "${CAKE_CURRENT_DIRECTORY}/${_CAKE_PROJECT_FILE_NAME}")
        set(CAKE_PROJECT_DIR "${CAKE_CURRENT_DIRECTORY}" CACHE INTERNAL "" FORCE)
      elseif(IS_DIRECTORY "${_ENV_CAKE_PROJECT_DIR}")
        set(CAKE_PROJECT_DIR "${_ENV_CAKE_PROJECT_DIR}" CACHE INTERNAL "" FORCE)
      else()
        message(FATAL_ERROR "[cake] Can't locate ${_CAKE_PROJECT_FILE_NAME}, either run from a directory that contains it or set the CAKE_PROJECT_DIR environment variable to an existing directory.")
      endif()
    else()
      # 1. attempt CMAKE_HOME_DIRECTORY
      # 2. attempt CAKE_PROJECT_DIR
      if(EXISTS "${CMAKE_HOME_DIRECTORY}/${_CAKE_PROJECT_FILE_NAME}")
        set(CAKE_PROJECT_DIR "${CMAKE_HOME_DIRECTORY}" CACHE INTERNAL "" FORCE)
      else()
        if(NOT CAKE_PROJECT_DIR)
          set(CAKE_PROJECT_DIR "${_ENV_CAKE_PROJECT_DIR}")
        endif()
        if(IS_DIRECTORY "${CAKE_PROJECT_DIR}")
          set(CAKE_PROJECT_DIR "${CAKE_PROJECT_DIR}" CACHE INTERNAL "" FORCE)
        else()
          message(FATAL_ERROR "[cake] Can't locate ${_CAKE_PROJECT_FILE_NAME}, either the top-level CMakeLists.txt directory should contain one or set the CAKE_PROJECT_DIR CMake or environment variable to an existing directory.")
        endif()
      endif()
    endif()
  endmacro()
